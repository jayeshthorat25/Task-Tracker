...
function UserTasks() {
  ...
  const {refreshTasks: parentRefreshTasks} = useOutletContext();
  ...
  const fetchUserTasks = useCallback(async () => {
    try {
      const response = await api.get(
        `admin/user/${user_id}/tasks`
      );
      setTasks(response.data);
    } catch (error) {
      console.error("Failed to fetch user tasks:", error.response.data.detail);
    }
  }, [user_id]);
  ...
  const FILTER_OPTIONS = ["all", "today", "week", "in progress", "pending", "completed"];
  const FILTER_LABELS = {
    all: "All Tasks",
    today: "Today's Tasks",
    week: "This Week",
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-4 md:mb-6 gap-3">
        <div className="min-w-0">
          <h1 className="text-xl md:text-3x1 font-bold text-gray-800 flex items-center gap-2">
            ...
            <span className="truncate">All Tasks of the selected user</span>
          </h1>
        </div>
      </div>
          ...
          {filterTasks.length === 0 ? (
            <div className="p-6 bg-white rounded-xl shadow-sm border border-purple-100 text-center">
              ...
              <p className="text-sm text-gray-500 mb-4">
                {filter === "all"
                  ? "The selected user has no tasks."
